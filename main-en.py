...
with gr.Blocks() as block_main:
    ...
    with gr.Tab("Home", visible=True, elem_id='home_tab') as home_tab:
        home_md = gr.Markdown('''## Welcome to KUMA's User Assistant!
                    
                    This tool is designed to help KUMA users.

                    To get started, enter the address, port, and API token in the fields above and click Connect to KUMA.

                    If everything is set up correctly, you will see additional tabs:

                    **Export** - allows you to export alerts, incidents, or correlation rules in CSV format

                    **Assets** - allows you to import assets in CSV format

                    **Backup/Restore** - allows you to perform backup and restore operations for KUMA

                    **Ananlyze** - allows you to analyze any KUMA resource in JSON format
                        ''')
    ...
    with gr.Tab("Assets", visible=False, elem_id="assets_tab") as assets_tab:
        ...
        gr.Markdown('''To import assets, select the tenant and upload a CSV file containing the assets.

                       The CSV file must include the following header: name,fqdn,ipAddresses,macAddresses,osName,osVersion

                       Each asset must have at least one of the two fields filled in: **fqdn** or **ipAddresses**

                       The fields **fqdn**, **ipAddresses**, **macAddresses** are arrays — use **;** to separate multiple values within a field
                    ''')
